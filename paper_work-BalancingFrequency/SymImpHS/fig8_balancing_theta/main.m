...
C_i         = zeros(length(phi_i),length(THETA1));
denominator = zeros(1,length(THETA1));
numerator   = zeros(1,length(THETA1));

for i = 1:length(phi_i)
  % fprintf(['i =' i '\n']);
  C_i(i,:) = m_i(i)*tan(phi_i(i)/2)^(2*n)*(n+cos(phi_i(i)))^2*ones(1,length(THETA1))
...
K_iL = zeros(1,length(THETA1));
K_iH = zeros(1,length(THETA1));
...
omega0 = ones(1,length(THETA1))*( n^2*(n^2-1)^2 * E*h^2 / (3*(1+mu)*rho*a^4) * U_phi/K_phi )^.5
...
figure(1);
hold on;
p(1)=plot(THETA2,freq0,'color',[0 0.4470 0.7410],'DisplayName','Perfect'); %'color',[0 0.4470 0.7410],'Marker','o','DisplayName','Perfect');
p(2)=plot(THETA2,freqL,'b','DisplayName','j = L mode');
p(3)=plot(THETA2,freqH,'r--','DisplayName','j = H mode');
...
saveas(figure(1),['figures/fig8_n' num2str(n) fsign '.png']);
