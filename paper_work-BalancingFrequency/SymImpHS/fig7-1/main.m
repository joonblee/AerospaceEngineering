...
hOa = 0.01; % a/h
a   = ((3*M_HS/(2*pi*rho))/(3*hOa+(hOa)^3/4))^(1/3); % radius
% 1/2 rho [ 4/3 pi (a+h/2)^3 - 4/3 pi (a-h/2)^3 ] = 5 - M_imp kg
%%%a_PF  = ((3*5/(2*pi*rho))/(3*hOa+(hOa)^3/4))^(1/3); % radius of perfect HS of 5 kg.
%%% PF: perfect

h     = a*hOa; % width
%%%h_PF  = a_PF*hOa;

rho_ = (M_HS+0.2)/(2/3*pi*a^3*( (1+hOa/2)^3 - (1-hOa/2)^3 ));
...
omega0_PF = double(( n.^2.*(n.^2-1).^2 * E*h^2 ./ (3*(1+mu)*rho_*a^4) .* U_phi./K_phi ).^.5)
...
freq0_PF = double(omega0_PF / (2*pi))

figure(1);
p(1)=plot(n,freq0,'o','color',[0 0.4470 0.7410],'DisplayName','PF mod M'); %'color',[0 0.4470 0.7410],'Marker','o','DisplayName','Perfect');
hold on;
p(2)=plot(n,freqL,'b+','MarkerSize',8,'DisplayName','j = L mode');
p(3)=plot(n,freqH,'r^','DisplayName','j = H mode');

p(4)=plot(n,freq0_PF,'-m','DisplayName','PF 5kg');
...
saveas(figure(1),['figures/fig7' fsign '_p' fphi '_t' ftheta '.png']);
