...
fig;
hold on;
p(1)=plot(PHI,Freq0,'k','LineWidth',1.1,'DisplayName','perfect'); %'color',[0 0.4470 0.7410],'Marker','o','DisplayName','Perfect');
p(2)=plot(PHI,Freq_Mass_L,'b','LineStyle','--','LineWidth',1.1,'DisplayName','1M (j = L)');
p(3)=plot(PHI,Freq_Mass_H,'color',[0.58 0 0.83],'LineStyle','--','LineWidth',1.1,'DisplayName','1M (j = H)');
p(4)=plot(PHI,Freq_Hole_L,'r','LineStyle',':','LineWidth',1.5,'DisplayName','1H (j = L)');
p(5)=plot(PHI,Freq_Hole_H,'color',[1.0 0.5 0.0],'LineStyle',':','LineWidth',1.5,'DisplayName','1H (j = H)');
...
legend(p(1:5),'Location','NorthEast'); legend boxoff;

saveas(fig,['figures/1imp_phi.png']);
saveas(fig,['figures/1imp_phi.pdf']);
